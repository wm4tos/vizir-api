...
describe('Testando service de ligações', () => {
  ...
  it('Deve receber um objeto com a ligação cadastrada', () => {
    return NewCall({
      origin: 20,
  ...
  it('Deve receber a mensagem "Parâmetros inválidos."', () => {
    return NewCall({ origin: 23 })
      .catch((call) => {
        expect(call).to.be.equal('Parâmetros inválidos.')
      });
  });
  it('Deve receber todas as ligações', () => {
    return GetCalls()
      .then((calls) => {
        expect(calls).to.be.a('array').to.have.length(4);
      });
  });
  it('Deve receber um array vazio', () => {
    return GetCalls({ origin: 9999 })
      .then((calls) => {
        expect(calls).to.be.an('array');
        expect(calls).to.be.empty;
      });
  });
  it('Deve receber uma ligação', () => {
    return GetCall()
      .then((call) => {
        expect(call).to.have.property('origin');
  ...
  it('Deve receber um objeto vazio', () => {
    return GetCall({ origin: 9999 })
      .then((call) => {
